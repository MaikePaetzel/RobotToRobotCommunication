package com.bachelorthesis.lib;

// Quelle: http://mirlastfm.googlecode.com/svn-history/r3/trunk/CoMIRVA/src/comirva/audio/util/MFCC.java
// 29.03. 14:14
...
public class MFCC
{
   ...
   */
  public MFCC(float sampleRate) throws IllegalArgumentException, RuntimeException
  {
    this(sampleRate, 1024, 20, true, 20.0, 16000.0, 40);
  }
   ...
   * @throws IllegalArgumentException raised if mehtod contract is violated
   */
  public MFCC(float sampleRate, int windowSize) throws IllegalArgumentException
  {
      this(sampleRate, windowSize, 20, true, 20.0, 16000.0, 40);
  }
 ...
 private Matrix getMelFilterBanks()
   ...
   *                window
   * @throws java.io.IOException if there are any problems regarding the inputstream
   * @throws IllegalArgumentException raised if mehtod contract is violated
   */
  public Vector process(AudioPreProcessor in) throws IllegalArgumentException, IOException
  {
    ...
    while (samplesRead == hopSize)
    {
      ...
      for (int i = hopSize; i < windowSize; i++)
      {
    	  inputData[i] = 0;
      }
      //read new data
