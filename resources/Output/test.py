...
ax = plt.subplot(111)
ax.plot([(i*8/22050.0) for i in x], [float(elem)/float(2**15) for elem in daten], 'r-')
ax.plot([(i*8/22050.0) for i in x], [float(elem)/float(2**15) for elem in boolean], 'g-')
ax.plot([(i*8/22050.0) for i in x], [float(elem)/float(2**15) for elem in average], 'b-')
#ax.plot(date_neg_x, date_neg, 'm+')
ax.plot(average_neg_x, average_neg, 'yo')
ax.set_xlabel('Zeit in Sekunden')
ax.set_ylabel('Ampliute')
...
for j in splitx:
	...
	if(len(splitx2) == 2):
		...
		ax.plot([b/22050.0 for b in blub], [-0.05 for i in blub], 'k-', linewidth=10.0)
